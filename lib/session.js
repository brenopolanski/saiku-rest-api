...
function SessionRequest(options) {
	...
	this._url = this._options.host + settings.REST_URL + 'session';
...
SessionRequest.prototype.createSession = function() {
	...
	function(error, response, body) {
		if (!error && response.statusCode === 200) {
			setSessionLocalStorage(error, response, self._url);
		}
		else {
...
function setSessionLocalStorage(error, response, url) {
